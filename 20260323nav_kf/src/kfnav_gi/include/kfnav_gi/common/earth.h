...
public:
    /* 正常重力计算 */
    static auto gravity(const Vector3d &blh) -> double {
    ...
    /* 计算子午圈半径和卯酉圈半径 */
    static auto meridianPrimeVerticalRadius(double lat) -> Eigen::Vector2d {
        double tmp, sqrttmp;
    ...
    static auto qne(const Vector3d &blh) -> Quaterniond {
        Quaterniond quat;
    ...
    /* 从n系到e系转换四元数得到纬度和经度 */
    static auto blh(const Quaterniond &qne, double height) -> Vector3d {
        return {-2 * atan(qne.y() / qne.w()) - M_PI * 0.5, 2 * atan2(qne.z(), qne.w()), height};
    }

    /* 大地坐标(纬度、经度和高程)转地心地固坐标 */
    static auto blh2ecef(const Vector3d &blh) -> Vector3d {
    ...
    /* 地心地固坐标转大地坐标 */
    static auto ecef2blh(const Vector3d &ecef) -> Vector3d {
    ...
    /* n系相对位置转大地坐标相对位置 */
    static auto DRi(const Vector3d &blh) -> Matrix3d {
    ...
    /* 大地坐标相对位置转n系相对位置 */
    static auto DR(const Vector3d &blh) -> Matrix3d {
    ...
    static auto local2global(const Vector3d &origin, const Vector3d &local) -> Vector3d {
    ...
    static auto global2local(const Vector3d &origin, const Vector3d &global) -> Vector3d {
        Vector3d ecef0 = blh2ecef(origin);
        Matrix3d cn0e = cne(origin);
    ...
    static auto ecef2nedv(const Vector3d &origin, const Vector3d &vecef) -> Vector3d {
        Matrix3d cn0e = cne(origin);
        return cn0e.transpose() * vecef;
    }

    static auto local2global(const Vector3d &origin, const Pose &local) -> Pose {
        Pose global;
    ...
    static auto global2local(const Vector3d &origin, const Pose &global) -> Pose {
        Pose local;
    ...
    /* 地球自转角速度投影到e系 */
    static auto wiee() -> Vector3d {
        return {0, 0, WGS84_WIE};
    }

    /* 地球自转角速度投影到n系 */
    static auto wien(double lat) -> Vector3d {
        return {WGS84_WIE * cos(lat), 0, -WGS84_WIE * sin(lat)};
    }

    static auto wien(const Vector3d &origin, const Vector3d &local) -> Vector3d {
    ...
    /* n系相对于e系转动角速度投影到n系 */
    static auto wenn(const Eigen::Vector2d &rmn, const Vector3d &blh, const Vector3d &vel) -> Vector3d {
        return {vel[1] / (rmn[1] + blh[2]), -vel[0] / (rmn[0] + blh[2]), -vel[1] * tan(blh[0]) / (rmn[1] + blh[2])};
    }

    static auto wenn(const Vector3d &origin, const Vector3d &local, const Vector3d &vel) -> Vector3d {
        Vector3d global = local2global(origin, local);
